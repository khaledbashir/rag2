...
| B14 | **Repeat Clients Report skill** — Scout fires on "top 10 repeat technology clients" / "lifetime revenue by [vertical]" → produces single-sheet Excel matching Jireh's `ANC - Technology Repeat Clients` template (Client · First-Year · Lifetime Revenue · Lifetime Margin %). | ✅ | **Verified 2026-04-26 19:59 UTC.** Production URL `https://proposals.anc.com/api/jireh-reports/repeat-clients?vertical=TECHNOLOGY&top=10` returns valid XLSX. Opened `output/spreadsheet/ANC-Technology-Repeat-Clients-2026-production-patched.xlsx` and diffed against Jireh template `ANC - Technology Repeat Clients 2026 2027 2028 (April 2026) (1).xlsx`: sheet name, C3:F3 headers, blank rows/cols, no merges, hidden gridlines, header fill/font, row heights, and column widths all pass. Skill `repeat-clients-report` (`54288e81-63d5-4b4d-9cb6-a0bf0d5b08e8`) active; tool `generate-repeat-clients-report` (`8acf4271-f0a6-46fa-9b0c-cceb55cb9b6a`) active. |
| B15 | **Account LTV Report skill** — Scout fires on "lifetime value of [Company]" / "Hankook-style report on [Company]" → produces Hankook-style Excel (Summary + Deal History from CRM revenue-bearing WON opps). | ✅ | **Verified 2026-04-26 19:59 UTC.** Production URL `https://proposals.anc.com/api/jireh-reports/account-ltv?company=Hankook` returns valid XLSX with `x-won-opps: 32`, combining `Hankook Tire America Corp.` + `Hankook Tire Canada Corp.` and skipping zero-amount rows. Opened `output/spreadsheet/Hankook-LTV-production-patched.xlsx` and diffed Summary against `Hankook_Lifetime_Value_ANC_Final.xlsx`: title/merge structure, gridlines, row heights, column widths, key fills/fonts pass. Output: `$33.4M`, `2016–2024`, `32 deals`. Known data limitation: no MLB Teams tab because CRM does not store team-level sub-breakdowns; Jireh's `$35M+` includes pre-2016/future/hospitality extras outside current CRM fields. Skill `account-ltv-report` (`4b932931-a798-4ea1-921e-a6f7367ec596`) active; tool `generate-account-ltv-report` (`a1d07377-2080-425b-b7a2-056eaa57dd26`) active. |
