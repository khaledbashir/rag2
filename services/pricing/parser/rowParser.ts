...
export function parseAllRows(
  ...
  for (let i = headerRowIdx + 1; i < data.length; i++) {
    ...
    const isGrandTotal = labelNorm.includes("grand total") || labelNorm === "document total" || labelNorm.includes("sub total (bid form)") || labelNorm === "project total";
    const isAlternateHeader = isAlternateAddToCost || isAlternateDeduct || (looksLikeAlternateHeader && !hasColumnHeaders);
    const isAlternateLine = labelNorm.startsWith("alt ") || labelNorm.startsWith("alt-") || labelNorm.startsWith("alt:") || labelNorm.startsWith("alternate ");
