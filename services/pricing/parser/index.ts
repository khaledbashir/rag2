 ...
 *
 * This is the orchestrator module. Sub-modules:
 *   - columnDetection: finds pricing column headers
 *   - rowParser: converts raw Excel rows into typed RawRow[]
 *   - boundaryDetection: identifies table start/end boundaries
 *   - tableExtraction: converts boundaries into PricingTable objects
 *   - validation: builds validation reports
 */
...
import { findColumnHeaders, findHeaderRowIndex, deriveBestShiftedColumnMap } from "./columnDetection";
import { parseAllRows } from "./rowParser";
import { findTableBoundaries, buildSingleTableBoundary, findGlobalDocumentTotal } from "./boundaryDetection";
import { extractTable, prependSyntheticRollupTable } from "./tableExtraction";
...
function parsePricingTablesInner(
  ...
  let columnMap: ColumnMap | null = findColumnHeaders(data);
  if (!columnMap) {
