            ...
            <main className={`flex-1 min-h-0 overflow-hidden grid transition-all duration-500 ease-in-out ${
                ...
                <section className="relative min-w-0 min-h-0 bg-zinc-100 dark:bg-zinc-950 overflow-hidden flex flex-col p-3">
                    <div className="shrink-0 mb-2 flex items-center justify-between rounded-md border border-border bg-background/90 px-3 py-2 text-[11px] text-muted-foreground">
                        <span>Live workbook — edits sync into estimator state.</span>
                        {uiFeedback ? (
                            <span className={uiFeedback.tone === "error" ? "truncate text-destructive" : uiFeedback.tone === "success" ? "truncate text-emerald-600" : "truncate text-[#0A52EF]"}>
                                {uiFeedback.message}
                            </span>
                        ) : workbookSyncMessage ? (
