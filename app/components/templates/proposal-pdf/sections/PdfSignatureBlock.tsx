...
const PdfSignatureBlock = ({ colors, receiverName, receiverAddress, signatureBlockText }: PdfSignatureBlockProps) => (
    <div data-preview-section="signature" className="mt-4 break-inside-avoid" style={{ pageBreakInside: 'avoid', breakInside: 'avoid' }}>
        <div className="text-[11px] leading-snug text-justify mb-3 break-inside-avoid whitespace-pre-wrap" style={{ color: colors.textMuted }}>
            {(signatureBlockText || "").trim() || DEFAULT_SIGNATURE_BLOCK_TEXT}
        </div>
        <h4 className="font-bold text-[11px] uppercase mb-3 border-b-2 pb-0.5 break-inside-avoid" style={{ borderColor: colors.text, color: colors.text }}>
            ...
            {[
                ...
                <div key={idx} className="space-y-2 break-inside-avoid">
                    <div className="break-inside-avoid">
                        <div className="font-bold text-[11px]" style={{ color: colors.text }}>{party.title}</div>
                        {party.address && (
                    ...
                    <div className="break-inside-avoid">
                        <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: colors.textMuted }}>By:</div>
                        <div className="h-6 border-b-2" style={{ borderColor: colors.border }} />
                    </div>
                    <div className="break-inside-avoid">
                        <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: colors.textMuted }}>Title:</div>
                        <div className="h-5 border-b" style={{ borderColor: colors.border }} />
                    </div>
                    <div className="break-inside-avoid">
                        <div className="text-[10px] uppercase tracking-wide mb-0.5" style={{ color: colors.textMuted }}>Date:</div>
                        <div className="h-5 border-b" style={{ borderColor: colors.border }} />
                    </div>
                </div>
            ))}
