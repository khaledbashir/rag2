#!/usr/bin/env bash
# Private-review / approved-group Closed-Won report sender.
#
...
#
# Usage: anc-crm-closed-won-report.sh <last7|monthToDate> [extra-query-string] [review|group]
set -euo pipefail
...
case "${PERIOD}" in
  last7|monthToDate) ;;
  *) echo "Invalid period: ${PERIOD}" >&2; exit 1 ;;
esac

case "${DELIVERY}" in
  review|group) ;;
  *) echo "Invalid delivery: ${DELIVERY}" >&2; exit 1 ;;
esac

ENDPOINT="https://proposals.anc.com/api/crm-reports/closed-won/send?period=${PERIOD}&delivery=${DELIVERY}${EXTRA_QUERY}"
...
# Recipient selection is enforced by the endpoint. Review means Jireh only;
# group means the configured list and remains locked until his approval is
# recorded in the deployment configuration.
PAYLOAD='{}'
...
for attempt in $(seq 1 "${ATTEMPTS}"); do
  echo "$(date -Is) sending closed-won report period=${PERIOD} delivery=${DELIVERY} attempt=${attempt}/${ATTEMPTS}"
  ...
  if [[ "${HTTP_CODE}" == "200" && "${BODY}" == *'"ok":true'* ]]; then
    echo "$(date -Is) closed-won report period=${PERIOD} delivery=${DELIVERY} OK (http ${HTTP_CODE})"
    record_status "delivery=${DELIVERY} ok http=${HTTP_CODE} attempt=${attempt}"
    exit 0
  fi

  echo "$(date -Is) closed-won report period=${PERIOD} delivery=${DELIVERY} FAILED http=${HTTP_CODE} attempt=${attempt}/${ATTEMPTS}" >&2
...
echo "$(date -Is) closed-won report period=${PERIOD} delivery=${DELIVERY} GAVE UP after ${ATTEMPTS} attempts — no email was sent" >&2
record_status "delivery=${DELIVERY} FAILED http=${HTTP_CODE} after ${ATTEMPTS} attempts"
exit 1
